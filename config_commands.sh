...
if [ -z "$db" ]; then
     echo "You must pass in six variables, admin username, password, host, database name, database owner, and database owner password."
     exit
fi
...
#Insert Gages into Database as gage.
ogr2ogr -overwrite -progress -f "PostGreSQL" PG:"host=$host user=$owner password=$ownerpass dbname=$db" -nln "gage" -nlt PROMOTE_TO_MULTI -lco "GEOMETRY_NAME=the_geom" -lco "PRECISION=NO" -a_srs "EPSG:4269"   NHDPlusV21_National_Flattened.gdb Gage

# Index Gages.
psql -c "CREATE INDEX ON gage (comid); CREATE INDEX ON gage (reachcode); CREATE INDEX ON gage (eventdate); CREATE INDEX ON gage (reachsmdat); CREATE INDEX ON gage (reachresol); CREATE INDEX ON gage (measure); CREATE INDEX ON gage (eventtype); CREATE INDEX ON gage (state); CREATE INDEX ON gage (dasqkm);" postgresql://$owner:$ownerpass@$host:5432/$db

# Insert Sink as sink
ogr2ogr -overwrite -progress -f "PostGreSQL" PG:"host=$host user=$owner password=$ownerpass dbname=$db" -nln "sink" -nlt PROMOTE_TO_MULTI -lco "GEOMETRY_NAME=the_geom" -lco "PRECISION=NO" -a_srs "EPSG:4269"   NHDPlusV21_National_Flattened.gdb Sink
...
# Insert LandSea as landsea
ogr2ogr -overwrite -progress -f "PostGreSQL" PG:"host=$host user=$owner password=$ownerpass dbname=$db" -nln "landsea" -nlt PROMOTE_TO_MULTI -lco "GEOMETRY_NAME=the_geom" -lco "PRECISION=NO" -a_srs "EPSG:4269"   NHDPlusV21_National_Flattened.gdb LandSea

# Index landsea
psql -c "CREATE INDEX ON landsea (LANDSEAID); CREATE INDEX ON landsea (LAND);" postgresql://$owner:$ownerpass@$host:5432/$db

# Insert NHDWaterbody as nhdwaterbody
ogr2ogr -overwrite -progress -f "PostGreSQL" PG:"host=$host user=$owner password=$ownerpass dbname=$db" -nln "nhdwaterbody" -nlt PROMOTE_TO_MULTI -lco "GEOMETRY_NAME=the_geom" -lco "PRECISION=NO" -a_srs "EPSG:4269"   NHDPlusV21_National_Flattened.gdb NHDWaterbody
...
# Insert NHDArea as nhdarea
ogr2ogr -overwrite -progress -f "PostGreSQL" PG:"host=$host user=$owner password=$ownerpass dbname=$db" -nln "nhdarea" -nlt PROMOTE_TO_MULTI -lco "GEOMETRY_NAME=the_geom" -lco "PRECISION=NO" -a_srs "EPSG:4269"   NHDPlusV21_National_Flattened.gdb NHDArea
...
# Insert NHDPoint as nhdpoint
ogr2ogr -overwrite -progress -f "PostGreSQL" PG:"host=$host user=$owner password=$ownerpass dbname=$db" -nln "nhdpoint" -nlt PROMOTE_TO_MULTI -lco "GEOMETRY_NAME=the_geom" -lco "PRECISION=NO" -a_srs "EPSG:4269"   NHDPlusV21_National_Flattened.gdb NHDPoint
...
# Insert NHDLine as nhdline
ogr2ogr -overwrite -progress -f "PostGreSQL" PG:"host=$host user=$owner password=$ownerpass dbname=$db" -nln "nhdline" -nlt PROMOTE_TO_MULTI -lco "GEOMETRY_NAME=the_geom" -lco "PRECISION=NO" -a_srs "EPSG:4269"   NHDPlusV21_National_Flattened.gdb NHDLine
...
# Insert HUC12 as huc12
ogr2ogr -overwrite -progress -f "PostGreSQL" PG:"host=$host user=$owner password=$ownerpass dbname=$db" -nln "huc12" -nlt PROMOTE_TO_MULTI -lco "GEOMETRY_NAME=the_geom" -lco "PRECISION=NO" -a_srs "EPSG:4269"   NHDPlusV21_National_Flattened.gdb HUC12

# Index huc12
psql -c "CREATE INDEX ON huc12 (HUC_8); CREATE INDEX ON huc12 (HUC_10); CREATE INDEX ON huc12 (HUC_12); CREATE INDEX ON huc12 (ACRES); CREATE INDEX ON huc12 (NCONTRB_A); CREATE INDEX ON huc12 (HU_10_GNIS); CREATE INDEX ON huc12 (HU_12_GNIS); CREATE INDEX ON huc12 (HU_10_DS); CREATE INDEX ON huc12 (HU_10_NAME); CREATE INDEX ON huc12 (HU_10_MOD); CREATE INDEX ON huc12 (HU_10_TYPE); CREATE INDEX ON huc12 (HU_12_DS); CREATE INDEX ON huc12 (HU_12_NAME); CREATE INDEX ON huc12 (HU_12_MOD); CREATE INDEX ON huc12 (HU_12_TYPE); CREATE INDEX ON huc12 (META_ID); CREATE INDEX ON huc12 (STATES); CREATE INDEX ON huc12 (GlobalID); CREATE INDEX ON huc12 (SHAPE_Leng); CREATE INDEX ON huc12 (GAZ_ID); CREATE INDEX ON huc12 (WBD_Date); CREATE INDEX ON huc12 (VPUID); CREATE INDEX ON huc12 (HUC_2); CREATE INDEX ON huc12 (HUC_4); CREATE INDEX ON huc12 (HUC_6);" postgresql://$owner:$ownerpass@$host:5432/$db

# Insert Simplified Catchments into Database as Catchmentsp.
ogr2ogr -overwrite -progress -f "PostGreSQL" PG:"host=$host user=$owner password=$ownerpass dbname=$db" -nln "catchmentsp" -nlt PROMOTE_TO_MULTI -lco "GEOMETRY_NAME=the_geom" -lco "PRECISION=NO" -a_srs "EPSG:4269"   NHDPlusV21_National_Flattened.gdb CatchmentSP
...
# Insert Catchments into Database as Catchment.
ogr2ogr -overwrite -progress -f "PostGreSQL" PG:"host=$host user=$owner password=$ownerpass dbname=$db" -nln "catchment" -nlt PROMOTE_TO_MULTI -lco "GEOMETRY_NAME=the_geom" -lco "PRECISION=NO" -a_srs "EPSG:4269"   NHDPlusV21_National_Flattened.gdb Catchment
